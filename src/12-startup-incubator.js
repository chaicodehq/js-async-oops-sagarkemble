...
export class Startup {
  ...
  constructor(name, founder, domain) {
    const domainArr = ["fintech", "edtech", "healthtech", "foodtech"];
    const isValidDomain = domainArr.some((e) => e === domain);
    if (!isValidDomain)
      throw new Error(
...
export class Incubator {
  ...
  async admitStartup(startup) {
    return new Promise((resolve, reject) => {
      ...
      else if (this.#startups.some((s) => s.startup.name === startup.name))
        reject(new Error("Startup already admitted!"));
      else if (this.#startups.length >= this.maxStartups)
  ...
  removeStartup(name) {
    const index = this.#startups.findIndex((e) => e.startup.name === name);
  ...
  async assignMentor(startupName, mentor) {
    return new Promise((resolve, reject) => {
      const isValidStartUp = this.#startups.some(
        (e) => e.startup.name === startupName,
      );
      if (!isValidStartUp) reject(new Error("Startup not found!"));
      else {
        this.#mentors.push({
          startupName,
          mentor,
  ...
  async conductDemo(startupName) {
    return new Promise((resolve, reject) => {
      ...
      if (!startupObj) reject(new Error("Startup not found!"));
      else {
  ...
  async batchProcess(startups) {
    const startupsArr = startups.map((e) => this.admitStartup(e));
    return Promise.allSettled(startupsArr);
  }

  getStartupsByDomain(domain) {
    return this.#startups
      .filter((e) => e.startup.domain === domain)
      .map((e) => e.startup);
  }
...
export async function runDemoDay(incubator) {
  const demoArr = [];
  for (const startup of incubator) {
    demoArr.push(incubator.conductDemo(startup.name));
  }
  const results = await Promise.allSettled(demoArr);
  return {
    incubator: incubator.name,
