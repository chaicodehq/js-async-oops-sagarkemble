...
export function assignRider(order) {
  return new Promise((res, rej) => {
    const riderArr = ["Rahul", "Priya", "Amit", "Neha", "Vikram"];
    if (!order || order.status !== "confirmed") {
      rej(new Error("Order not confirmed yet!"));
    }
    const riderIndex = Math.floor(Math.random() * riderArr.length);
    res({ ...order, rider: riderArr[riderIndex], status: "assigned" });
  });
}
...
export function processMultipleOrders(orderList) {
  const promises = orderList.map((e) => processDelivery(e.restaurant, e.items));
